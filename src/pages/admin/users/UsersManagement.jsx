...
const UsersManagement = () => {
  const { getRoleDisplayName, isAdmin } = useRole()
  ...
  // Delete Confirmation
  const [deleteConfirm, setDeleteConfirm] = useState({ isOpen: false, userId: null, userName: null })
  ...
  const getRoleBadgeColor = (role) => {
    const colors = {
      [UserRole.ADMIN]: 'rgb(139, 92, 246)',
      [UserRole.MANAGER]: 'rgb(59, 130, 246)',
      [UserRole.USER]: 'rgb(107, 114, 128)'
    }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          <div className="flex flex-col gap-4">
            ...
            {/* Filter Button and Optional Filter Panel */}
            <div>
              <button
              ...
              >
                ...
                Filters {showFilters && <FiX className="w-4 h-4 ml-1" />}
              </button>

              {showFilters && (
                <div className="mt-4 p-4 rounded-lg bg-gray-50 border" style={{ borderColor: 'var(--border-color)' }}>
          ...
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p style={{ color: 'var(--text-secondary)' }}>
                {searchQuery ? 'Try adjusting your search query' : 'Create your first user'}
              </p>
            </div>
          ) : (
            <>
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-1">
                            ...
                            <span
                              ...
                              style={{ color: getStatusBadgeColor(user.status) }}
                            >
                              {user.status}
                            </span>
                          </div>
                            ...
                            {isAdmin() && (
                              <>
                                ...
                                <button
                                  ...
                                  className="p-2 rounded-lg transition-colors"
                                  style={{
                                    backgroundColor: user.status === 'active' ? 'rgba(239, 68, 68, 0.1)' : 'rgba(34, 197, 94, 0.1)',
                                    color: user.status === 'active' ? 'rgb(239, 68, 68)' : 'rgb(34, 197, 94)'
                                  }}
                                  title={user.status === 'active' ? 'Deactivate user' : 'Activate user'}
                                >
                                  {user.status === 'active' ? <FiXCircle className="w-4 h-4" /> : <FiCheckCircle className="w-4 h-4" />}
                                </button>
                                {user.id !== currentUser?.id && (
                                  <button
