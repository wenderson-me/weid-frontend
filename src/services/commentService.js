...
class CommentService {
  ...
  _handleError(error, defaultMessage) {
    // If the error has an API response
    if (error.response) {
      // Check for specific errors
      if (error.response.status === 403) {
        error.permissionDenied = true; // Flag to identify permission errors
      ...
      // Use the API message or default message
      return new Error(error.response.data?.message || defaultMessage);
    }

    // If the error is a connection error
    if (error.request) {
